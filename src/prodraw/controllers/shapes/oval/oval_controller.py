...
class OvalController:
    """Bridges raw Tkinter mouse events and the Oval model/view.
    ...
    def __init__(self, canvas: Canvas, figures: dict, get_bg: Callable[[], str]):
        ...
        self.view = OvalView(canvas)
        self.current: Oval = None
    ...
    def unbind(self):
        """Detach mouse event handlers and clear any drawn ovals.
    ...
    def _on_press(self, event: Event):
        ...
        A fresh Oval instance is created per press — no shared
        class-level state between draws."""
        self.current = Oval(bg=self.get_bg())
        self.current.start(event.x, event.y)

    def _on_drag(self, event: Event):
        """Step 2: mouse movement updates the in-progress oval's
